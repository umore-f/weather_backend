...
router.get('/vc_hours',async (req,res)=>{
    try{
        const { location } = req.query;
        const response = await axios.get(`https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${location.lat},${location.lon}/today?key=${process.env.VC_PRIVATE_KEY}&lang=zh_cn&unitGroup=metric&include=current`, {
            params: {  },
        });
        res.json(renameFields(response.data));
    } catch (error) {
