 ...
 * @returns {Promise<number>} 估计的真值
 */
async function selfConsistentBaseline(city, field) {
    const forecasts = await getNextWeather(city);
    if (!forecasts || forecasts.length === 0) {
        throw new Error(`城市 ${city} 无预报数据，无法计算自洽真值`);
    }
...
async function getSelfConsistentBaseline(city) {
    ...
    for (const field of FIELDS_CAL) {
        try {
            fieldBaselines[field] = await selfConsistentBaseline(city, field);
        } catch (error) {
